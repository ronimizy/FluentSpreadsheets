...
public static class ComponentFactory
{
    ...
    public static IComponent Label<T>(Func<Index, T> factory, string? format, IFormatProvider? formatProvider) where T : IFormattable
        => new CellAwareComponent(x => factory.Invoke(x).ToString(format, formatProvider));
    
    public static IComponent Label<T>(Func<Index, T> factory, IFormatProvider? formatProvider) where T : IFormattable
        => new CellAwareComponent(x => factory.Invoke(x).ToString(null, formatProvider));
